...
export default function Home() {
  ...
  // Hook para carregar filmes e gêneros
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
  ...
  for (const nameTry of opts) {
    const searchTerm: string = nameTry.toString(); // garante string
    const found = lowerGenres.find(
      (lg: { id: number; name: string }) =>
        lg.name.toLowerCase().includes(searchTerm.toLowerCase())
    );
